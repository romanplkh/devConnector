...
//Load Validation of input Fields
...
router.get("/all", (req, res) => {
  Profile.find()
    ...
    .then(allprofiles => {
      if (!allprofiles) {
        errors.noprofile = "Threre are no profiles";
        return res.json(404).json(errors);
      }

      res.json(allprofiles);
    })
    .catch(() => res.status(404).json({ profiles: "Threre are no profiles" }));
});
...
router.post(
  ...
  (req, res) => {
    ...
    if (req.body.instagramm)
      profileFields.social.instagramm = req.body.instagramm;
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(user => {
      ...
      //ADD TO EXPERIENCE ARRAY IN MODEL
